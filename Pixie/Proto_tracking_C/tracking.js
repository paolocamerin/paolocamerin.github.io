...
function setup() {
  can = createCanvas(
    document.querySelector("img").offsetWidth - 16,
    document.querySelector("img").offsetHeight - 16
  );
  ...
  for (let i = 0; i < maxBubbles; i++) {
    ...
    bubbles.push(
      new reactiveElement(
        createVector(
          width / 2 + (width / 3) * cos(angle * i + PI / 8),
          (height / 5) * 2 + (height / 4) * sin(angle * i + PI / 8)
        ),
        70,
        null,
        "#378076"
    ...
    for (var i = 0; i < trackingData.length; i++) {
      ...
      let tp = createVector(
        map(
          ...
          0,
          capture.width,
          width / 2 - view.captureTransform.w,
          width / 2 + view.captureTransform.w
        ),
      ...
      rect(mainPoint.x, mainPoint.y, mainPoint.width, mainPoint.height);
    }
  }
...
class pointer {
  ...
  move(target) {
    //print("moving");
    this.p.x = lerp(this.p.x, target.x + 30 * cos(this.ang), 0.07);
    this.p.y = lerp(
      this.p.y,
      target.y + 30 * sin(this.ang + this.randomOffset),
      0.07
    );
    // print(this.p.x);
    this.ang += 0.05;
...
class reactiveElement {
  constructor(pos, dim, sound, col) {
    ...
    this.phaseIncr = random(0.01, 0.001);
    this.dOffset = 0;
    this.c = col;
    this.timer = 0;
    ...
    if (cursorPoint) {
      if (this.isTouching(cursorPoint) && timerState == "countDown") {
        if (this.timer < this.timeLimit) {
          if (brAn.activityLevel(pos) > 0.005) {
            for (let i = 0; i < 5; i++) {
              pnt.particles.push(new particle(createVector(pnt.p.x, pnt.p.y)));
            }

            // print(this.timer);
            this.dOffset += map(sin(frameCount / 10), -1, 1, -0.1, 0.3);
  ...
  display() {
    ...
    if (this.active) {
      ...
      for (let i = 0; i < this.bubbleRes; i++) {
        ...
        const r =
          map(
            noise(xoff, yoff, this.phase),
            0,
            1,
            20,
            40 + this.excitment * 10
          ) + this.dOffset;
        const x = r * cos(a * i);
        const y = r * sin(a * i);
...
class cameraView {
  ...
  viewRender(imageFeed) {
    image(
      ...
      width / 2,
      height / 2,
      this.captureTransform.w,
      this.captureTransform.h
    );
  }
}
